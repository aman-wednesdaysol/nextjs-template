...
export const { Types: searchActionTypes, Creators: searchActionCreators } = createActions({
  searchRequest: [SEARCH_PAYLOAD.SEARCH_TERM],
  searchMoreRequest: null,
  // data, offset, hasMore, append
  searchSuccess: [PAYLOAD.DATA, SEARCH_PAYLOAD.OFFSET, SEARCH_PAYLOAD.HAS_MORE, 'append'],
...
const handleSearchSuccess = (draft, action) => {
  ...
  draft[SEARCH_PAYLOAD.OFFSET] = action[SEARCH_PAYLOAD.OFFSET] ?? 0;

  draft[SEARCH_PAYLOAD.HAS_MORE] = !!action[SEARCH_PAYLOAD.HAS_MORE];
};
