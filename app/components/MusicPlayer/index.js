...
export function MusicPlayer({
  ...
  const { handlePrevious, handleNext } = useMusicNavigation({
    playlist,
    playlistIndex,
    onNext,
    onPrevious,
  ...
  const { handleProgressClick, progressPercentage } = useProgress({
    progressBarRef,
    duration,
    onSetCurrentTime,
    audioRef
  ...
  return (
    <PlayerContainer>
      ...
      <SongInfo>
        <AlbumArt>
          {artworkUrl ? <AlbumImage src={artworkUrl} alt={trackName} /> : <PlaceholderArt>🎵</PlaceholderArt>}
        </AlbumArt>
        <SongDetails>
