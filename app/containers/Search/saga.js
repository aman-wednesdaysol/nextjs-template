...
const getRawHeaderOffset = (headers) =>
  headers['x-next-offset'] ?? headers['X-Next-Offset'];

const isValidHeaderValue = (raw) =>
...
const inferNextOffset = (hasMore, offset, results, defaultNextOffsetIfFull) => {
  if (hasMore) return (offset ?? 0) + results.length;
...
const resolveNextOffset = (headerOffset, inferred) =>
  headerOffset ?? inferred.nextOffset;

const resolveHasMore = (headerOffset, inferred) =>
  headerOffset !== null ? true : inferred.hasMore;
...
const extractErrorMessage = (response) =>
  extractDataMessage(response) ?? response?.problem ?? DEFAULT_ERROR;
...
export function* handleSearchMore() {
  try {
    ...
    if (isBlankTerm(searchTerm)) return;
