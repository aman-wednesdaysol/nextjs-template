...
function hasSameId(a, b) {
  if (a.trackId != null) return a.trackId === b.trackId;
  if (a.id != null) return a.id === b.id;
  return false;
}

function isSameSong(a, b) {
  if (!a || !b) return false;
  if (hasSameId(a, b)) return true;
  return a.trackName === b.trackName && a.artistName === b.artistName;
}


export function findSongIndex(playlist, song) {
  if (!playlist || !song) return -1;
...
const handleAddToQueue = (draft, action) => {
  draft.queue = Array.isArray(action.queue)
    ? action.queue
    : [...draft.queue, action.queue];
};
...
const hasNextPlaylistSong = (draft) =>
  draft.playlist.length > 0 &&
  draft.playlistIndex < draft.playlist.length - 1;
...
const hasPreviousPlaylistSong = (draft) =>
  draft.playlist.length > 0 &&
  draft.playlistIndex > 0;
